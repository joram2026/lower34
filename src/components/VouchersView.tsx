...
export default function VouchersView({
  ...
  return (
    <div id="vouchers-view-container" className="space-y-6 max-w-md mx-auto animate-fade-in text-left">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button 
            ...
            type="button"
            onClick={onBack}
            className="p-2.5 rounded-full bg-white border border-zinc-200 text-zinc-650 hover:text-zinc-900 transition-colors cursor-pointer shadow-xs active:scale-95 shrink-0"
          ...
          <div>
            <h2 className="text-xl font-black tracking-tight text-zinc-900 flex items-center gap-2">
              <Gift size={20} className="text-[#008B47]" />
              <span>Incentives & Vouchers</span>
            </h2>
            <p className="text-xs text-zinc-500 font-medium">
              Redeem promotional allocation codes & manage reward benefits
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6 rounded-3xl border border-zinc-200/80 bg-white shadow-xs relative overflow-hidden space-y-4">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black uppercase tracking-wider text-zinc-900 flex items-center gap-1.5 font-mono">
              <Tag size={13} className="text-[#008B47]" />
              <span>Redeem Allocation Code</span>
            </span>
            <span className="text-[9.5px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
              Immediate Credit
            </span>
          </div>
          ...
          <form onSubmit={handleRedeem} className="space-y-3">
            <div className="relative">
              <input
                ...
                maxLength={30}
                className="w-full py-3 pl-4 pr-20 rounded-xl text-sm font-black font-mono tracking-wider transition-all outline-none border bg-zinc-50 border-zinc-200 text-zinc-900 placeholder:text-zinc-400 focus:bg-white focus:border-[#008B47] focus:ring-1 focus:ring-[#008B47]"
              />
              <button
                type="button"
                onClick={handlePaste}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-zinc-100 hover:bg-zinc-200 text-zinc-700 border border-zinc-200 transition-all cursor-pointer"
              >
                Paste
              </button>
            </div>
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-[#008B47] hover:bg-[#007038] active:scale-98 text-white font-bold text-xs uppercase tracking-wider shadow-sm transition-all cursor-pointer flex items-center justify-center gap-2 disabled:bg-zinc-150 disabled:text-zinc-400 disabled:cursor-not-allowed"
            >
              {isRedeeming ? (
                ...
                <>
                  <Sparkles size={15} />
                  <span>Claim Allocation Reward</span>
                </>
              )}
            </button>
          </form>
          ...
          <p className="text-[11px] text-zinc-400 leading-tight">
            Single-use promo codes are credited immediately to your primary or trade balance.
          </p>
        </div>
      </div>

      {/* Active VIP Passes & Reward Status Overview */}
      {/* Active Passes & Benefit Status Overview */}
      <div className="space-y-2.5">
        <h3 className="text-xs font-black uppercase tracking-wider text-zinc-600">
          ...
          <div className={`p-4 rounded-2xl border shadow-xs transition-all ${
            ...
            <div className="flex items-center justify-between gap-1 mb-1.5">
              <span className="text-[10px] font-black uppercase tracking-wider flex items-center gap-1 font-mono text-zinc-700">
                <Zap size={13} className={extraPassInfo.active ? 'text-[#008B47]' : 'text-zinc-400'} />
              ...
              <span className={`text-[8.5px] font-black uppercase px-1.5 py-0.2 rounded-full border ${
                ...
                {extraPassInfo.active ? 'ACTIVE' : 'STANDARD'}
              </span>
            </div>
            <p className="text-xs font-black font-mono text-zinc-900">
              {extraPassInfo.active ? extraPassInfo.text : 'Standard Tier'}
            </p>
            {extraPassInfo.active && (
              <p className="text-[9.5px] text-emerald-800 font-medium mt-0.5">
                Valid until: {extraPassInfo.dateStr}
              </p>
            )}
          </div>

          {/* Trade Capital Card */}
          <div className="p-4 rounded-2xl border border-zinc-200/80 bg-white shadow-xs text-zinc-800">
            <div className="flex items-center justify-between gap-1 mb-1.5">
              <span className="text-[10px] font-black uppercase tracking-wider flex items-center gap-1 font-mono text-zinc-700">
                <Coins size={13} className="text-[#008B47]" />
                <span>Trade Balance</span>
              </span>
              <span className="text-[8.5px] font-black uppercase px-1.5 py-0.2 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
                USD
              </span>
            </div>
      ...
      <div className="space-y-3">
        ...
        {loadingClaims ? (
          <div className="py-8 text-center text-xs text-zinc-600 flex flex-col items-center gap-2">
            <div className="w-5 h-5 border-2 border-[#008B47] border-t-transparent rounded-full animate-spin" />
          ...
          <div className="p-6 rounded-2xl border border-zinc-200 bg-white text-center space-y-2 select-none shadow-xs">
            <div className="w-10 h-10 rounded-2xl bg-zinc-100 text-zinc-600 flex items-center justify-center mx-auto">
              <Gift size={20} />
            </div>
            <h4 className="text-xs font-bold text-zinc-800">No vouchers redeemed yet</h4>
            <p className="text-[11px] text-zinc-500 max-w-[260px] mx-auto leading-relaxed">
              Enter an authorized promotional or partner allocation code above to claim balance credits.
            </p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {claimedVouchers.map((claim) => {
              ...
              return (
                <div 
                  key={claim.id}
                  className="p-3.5 rounded-2xl border border-zinc-200 bg-white hover:border-emerald-300 shadow-xs flex items-center justify-between gap-3 transition-all"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-xl bg-emerald-50 text-[#008B47] flex items-center justify-center shrink-0">
                    ...
                    <div className="min-w-0 text-left">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8.5px] font-bold px-1.5 py-0.2 rounded bg-emerald-50 text-emerald-800 border border-emerald-200">
                          Applied
                        </span>
                      </div>
      ...
      <div className="p-4 rounded-2xl border border-zinc-200/80 bg-zinc-50 text-zinc-800 text-xs space-y-2 shadow-xs">
        <div className="flex items-center gap-1.5 font-bold text-zinc-900 text-xs">
          <Sparkles size={14} className="text-[#008B47] shrink-0" />
          <span>Allocation & Incentive Distribution</span>
        </div>
        <ul className="text-[11px] space-y-1.5 text-zinc-600 list-disc list-inside leading-relaxed font-normal">
          <li>Check official announcements for flash voucher releases and seasonal reward drops.</li>
          <li>Affiliate network builders unlock automatic volume bonuses upon milestone completion.</li>
          <li>Reward grants are applied immediately with no wagering lock-in periods.</li>
        </ul>
      </div>
    </div>
  );
